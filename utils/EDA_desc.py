...
전체 자료 중 제가 구하고자 하는 것은 사회적 거리두기 기간과, 거리두기가 완화된 이후의 오프라인 프로모션 빈도수이므로 '일자'를 기준으로 기간을 2분할 하였습니다. 
    ...
    st.header('4. 분석 및 연구 결과')
    st.write('''
         
    1. 코로나19로 인한 영화 시장의 티켓 판매량은, 코로나19 규제 완화 이후 
    전 세계적으로 다시 증가했음을 알 수 있다.

    2. 상기 결론과 마찬가지로, 코로나19로 인해 위축되었던 오프라인 프로모션 시장은 
    규제 완화 이후 눈에 띄는 성장세를 보여주었다.

    3. 하지만, 넷플릭스(이하 OTT)의 성장세는 전세계적인 코로나 19의 발병 이전부터 
    최근(2023년 3월)까지 꾸준히 우상향하고 있다.

    4. 따라서, 영화계에서 언급하는 “코로나 19로 인해 급성장한 OTT와, 이로 인한 
    오프라인 영화 시장의 위축“ 간에 유의미한 상관 관계가 있다고 보기에는 어려움이 있다.

    5. 다만, 상세한 순익과 매출 등 기업의 민감한 정보에는 접근이 제한되었던 바, 
    실제 오프라인 시장의 회복세가 어떠한 지는 추가적인 분석이 필요해 보인다.
